...
const IS_VR_AVAILABLE = (AFRAME.utils.device ? AFRAME.utils.device.isMobile() : AFRAME.utils.isMobile()) || window.hasNonPolyfillWebVRSupport
  ...
  __updateIntersectObject () {
    ...
    if (intersects.length > 0) {
      ...
      intersects.every(item => {
        if (!intersect && item.object.el) {
          if(item.object.el.getAttribute('no-select')){
            return true;
          }
